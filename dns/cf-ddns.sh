...
# Place at:
# /usr/local/bin/cf-ddns.sh && chmod +x /usr/local/bin/cf-ddns.sh
...
if [ "$CFRECORD_TYPE" = "A" ]; then
  ...
  echo "$CFRECORD_TYPE specified is invalid, CFRECORD_TYPE can only be A(for IPv4)"
  exit 2
fi
...
WAN_IP_FILE=$PWD/cf-wan_ip_$CFRECORD_NAME.txt
